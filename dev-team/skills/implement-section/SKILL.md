...
If `$name` is `surface`, stop: the public surface is built by `/dev-team:finalize-package $pkg`, not by
this skill.
...
|---|---|---|
| Repo contract | `docs/architecture.md` | Fall back to the contract delta. If neither exists, that is a blocker — return it and name `/dev-team:plan-repo` or `/dev-team:map-project` as the fix. |
| Package contract | `docs/packages/$pkg/contract.md` | Blocker unless a contract delta names this section as new. Name `/dev-team:plan-package $pkg`. |
