...
3. **Interview rule.** Decide what you would ask — package boundaries the brief does not
   ...
   `Raised by: /dev-team:plan-repo (interview)` and **stop** with the stop message. Otherwise proceed.
...
4. **Repo contract.** Invoke `planning-templates` and read `references/repo-contract.md`, then
   ...
   becomes a stub scoped `repo` recommending `/dev-team:plan-change`, and the contract stays as it is.
...
6. **Return** your standard summary, ending with the next command:
   `/dev-team:plan-package <first package in dependency order that has no docs/packages/<pkg>/contract.md>`
