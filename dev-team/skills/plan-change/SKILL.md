...
1. **Name the plan.** Derive a short kebab-case slug from the change — it becomes a permanent
   directory name and the second argument to `/dev-team:implement-section`, so make it something you
...
4. **Interview rule.** Decide what you would ask — an ambiguity in the existing code that
   ...
   `Raised by: /dev-team:plan-change <slug> (interview)` and **stop** with the stop message. Otherwise
...
5. **Seed anything canonical that is missing.** Before designing the change, make sure the
   ...
   - **No `docs/packages/<pkg>/contract.md` for an affected package** → the package has never
     ...
     recommend `/dev-team:plan-package <pkg>` (document mode) before this change, unless the change
   ...
   recommend `/dev-team:map-project` rather than seeding half of it. If a canonical doc exists but the
   ...
   recommend `/dev-team:map-project`, which is the skill allowed to reconcile canonical docs with
   reality.
...
8. **Unify.** Read the design docs you delegated, read `references/integration.md`, and write
   ...
   That section is the input to `/dev-team:sync-plan`, so write it for someone with no memory of this
   run.
   ...
   `docs/followups.md`, so its `/dev-team:plan-package` or `/dev-team:implement-section` finds it.
...
- Never modify an existing canonical document. Creating a missing one per step 5 is required;
  ...
  `/dev-team:sync-plan`, after the code exists.
...
- If the change touches one section and no contracts or interfaces, say so in the return and
  ...
  `/dev-team:implement-section` expects an integration doc, so a minimal one is still required.
