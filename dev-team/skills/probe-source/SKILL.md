---
name: probe-source
description: Research one external data source for one package - check the credential, read the official docs, call the endpoints the section needs, and write the observed schema, pagination, limits, and error shapes to docs/packages/<pkg>/sources/<source>.md with a scrubbed sample and a re-runnable probe script. /dev-team:plan-package runs this itself for every section with a source; run it directly after an API changes, or to add a source after planning.
...
- One source per run. Several sources are several runs — or one `/dev-team:plan-package $pkg`, which
