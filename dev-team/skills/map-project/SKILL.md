...
only. Each package is then adopted by `/dev-team:plan-package <pkg>`, which on existing code runs in
...
3. **Interview rule.** This is where the package decomposition gets settled, and it is worth
   ...
   tagged `Raised by: /dev-team:map-project (interview)` and **stop** with the stop message — before
...
- Do not spawn designers and do not write package documents; `/dev-team:plan-package` does both, in
  document mode, one package at a time.
- Do not design improvements. If the code does something badly, that is a follow-up — the
  change that fixes it is a separate `/dev-team:plan-change` run.
